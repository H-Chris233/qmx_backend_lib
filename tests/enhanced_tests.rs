...
mod error_handling_tests {
    ...
    fn test_database_with_corrupted_files() {
        ...
        if result.is_err() {
            ...
            assert!(retry_result.is_ok(), "Database should initialize after removing corrupted files");
        } else {
    ...
    fn test_serialization_with_special_characters() {
        ...
        assert_eq!(restored_student.note(), "包含特殊字符的备注: {}[]()!@#$%^&*");
    }
}
...
mod performance_tests {
    ...
    fn test_large_student_creation() {
        ...
        for i in 0..1000 {
            let uid = manager.create_student(
                StudentBuilder::new(format!("Performance_Student_{}", i), ((i % 80) + 10) as u8)
                    .lesson_left((i % 50) + 1)
            ).unwrap();
        ...
        // 性能基准：应该在合理时间内完成
        assert!(creation_duration.as_secs() < 5, "创建1000个学生应该在5秒内完成");
    ...
    fn test_batch_operations_performance() {
        ...
        let filtered_students: Vec<_> = db.iter()
            .filter(|(_, s)| s.age() > 50 && s.lesson_left().unwrap_or(0) > 15)
            .collect();
        let query_duration = query_start.elapsed();
        
        println!("复杂查询耗时: {:?}, 找到{}个学生", query_duration, filtered_students.len());
    ...
    fn test_serialization_performance() {
        ...
        for i in 0..1000 {
            let mut student = Student::new();
            student.set_name(format!("Serialization_Test_Student_{}_with_long_name_containing_unicode_测试", i));
            student.set_age((i % 100) as u8);
            student.set_phone(format!("+86-{:04}-{:04}-{:04}", i % 1000, (i * 2) % 10000, (i * 3) % 10000));
            student.set_lesson_left((i % 100) + 1);
            student.set_note(format!("详细备注信息包含特殊字符 !@#$%^&*()_+ 和中文 测试备注 {}", i));
        ...
        println!("JSON大小: {} bytes ({:.2} MB)", json_size, json_size as f64 / 1024.0 / 1024.0);
        ...
        assert!(deserialize_duration.as_secs() < 3, "反序列化应该在3秒内完成");
    }
}
...
mod integration_tests {
    ...
    fn test_complete_workflow() {
        ...
        // 1. 创建学生
        let student_uid = manager.create_student(
        ...
        // 再次更新设置lesson_left
        let lesson_update = manager.update_student(
            student_uid,
            StudentUpdater::new().lesson_left(Some(18))
        );
        assert!(lesson_update.is_ok());
        
        // 4. 添加现金收入记录
        let income_uid = manager.record_cash(
    ...
    fn test_concurrent_operations() {
        ...
        for i in 0..10 {
            ...
            let handle = thread::spawn(move || {
                let result = manager_clone.create_student(
                    StudentBuilder::new(format!("并发学生_{}", i), 20 + (i % 50) as u8)
                );
                result.unwrap()
            });
            handles.push(handle);
